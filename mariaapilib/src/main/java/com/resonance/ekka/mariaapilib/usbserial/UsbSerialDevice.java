...
import com.resonance.ekka.mariaapilib.deviceids.CH34xIds;
import com.resonance.ekka.mariaapilib.deviceids.CP210xIds;
import com.resonance.ekka.mariaapilib.deviceids.FTDISioIds;
import com.resonance.ekka.mariaapilib.deviceids.PL2303Ids;
...
public abstract class UsbSerialDevice implements UsbSerialInterface
{
    ...
    public static UsbSerialDevice createUsbSerialDevice(UsbDevice device, UsbDeviceConnection connection, int iface)
    {
        ...
        if(FTDISioIds.isDeviceSupported(vid, pid))
            return null;//new FTDISerialDevice(device, connection, iface);
            ...
            return null;//new PL2303SerialDevice(device, connection, iface);
        else if(CH34xIds.isDeviceSupported(vid, pid))
            return null;//new CH34xSerialDevice(device, connection, iface);
        else if(isCdcDevice(device))
            return null;//new CDCSerialDevice(device, connection, iface);
        else
            return null;
    }
